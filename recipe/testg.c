...
static struct constraint_st    /* Table of Constraints */
{
    ...
    boolean computed;
    boolean inverse;
    unsigned long long prereq;    /* Must be earlier, must be <= bits in long long */
    long long lo,hi;
    char *id;
    int valtype;
    long long val;
    ...
    if (oneswitch)
    {
        for (i = 0; i < numkeys; ++i)
        {
		    ...
		    szb = (group_node*)val[i].hi;
                ...
                fprintf(f,"%lld %lld",val[i].lo,val[i].hi);
	    else
                fprintf(f,"%lld",val[i].lo);
        }
	...
	fprintf(f," %10" COUNTER_FMT_RAW " graphs : ",count);
        for (i = 0; i < numkeys; ++i)
        {
		    ...
		    if (compare_groupnodes(sza,szb))
		    {
                ...
                fprintf(f,"%s=%lld:%lld",ID(ki),val[i].lo,val[i].hi);
	    else
                fprintf(f,"%s=%lld",ID(ki),val[i].lo);
        }
	fprintf(f,"\n");
    }
}
...
static boolean
group_in_range(group_node *sz, long long lo, long long hi)
